...
function toThemeValueIfNeeded<T>(propKey: string, value: T, theme?: AppTheme) {
  ...
  } else if (theme?.letterSpacings && LINE_SPACING_KEYS.has(propKey) && isLetterSpacingThemeKeys(value, theme)) {
    return theme.letterSpacings[value]
  } else if (theme?.lineHeights && LINE_HEIGHT_KEYS.has(propKey) && isLineHeightThemeKeys(value, theme)) {
